...
interface ChallengeTask {
  ...
  time_end: string;
  point: number;
  status: 'done' | 'missed' | 'unchanged';
}

export default function ChallengePage() {
  const router = useRouter();
  const [user, setUser] = useState<{ id?: number; username?: string; full_name?: string; telegram_id?: string } | null>(null);
  const [loading, setLoading] = useState(true);
  const [authError, setAuthError] = useState<string | null>(null);
  const [challenges, setChallenges] = useState<ChallengeTask[]>([]);
  ...
  const fetchUserTasks = async (telegramId: string) => {
    try {
  ...
  const completeTask = async (taskId: number, status: 'done' | 'missed') => {
  ...
  const isTaskTimeValid = (startTime: string, endTime: string) => {
  ...
  const handleTaskClick = (task: ChallengeTask) => {
  ...
  const getTaskStatusText = (task: ChallengeTask) => {
        ...
        <div className="mb-8">
          ...
          <div className="space-y-3">
            {challenges.map((challenge) => {
              const isClickable = challenge.status === 'unchanged' && 
                                isTaskTimeValid(challenge.time_start, challenge.time_end);
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {statusText && (
                        <p className="text-xs text-red-400 mt-1">{statusText}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
